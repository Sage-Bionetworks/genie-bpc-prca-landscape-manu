...
eval_event_in_interval <- function(
    ...
    tol = 365.25 / 0.5 # half a day in years tolerance
) {
  ...
  dat_rtn %<>%
    group_by('record_id', all_of(var_block_group)) %>%
    summarize(
...
- I am excluding any period of followup where the medical oncologist did not yet declare HSPC or CRPC.  Excluding undelcared time blocks has a major impact because that is a common situation in our cohort.  - Once someone is declared as having CRPC I assume this condition is permanent (for example, if the next assessment is blank or differs, I ignore that and consider them castrate-resistant still).
