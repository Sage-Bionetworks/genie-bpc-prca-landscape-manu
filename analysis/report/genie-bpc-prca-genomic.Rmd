...
### R2.1.2b Panel coverage (table)

The following table shows similar information to the above figure: the number of patients, samples and genes covered by each panel.  
...
### R2.1.3 Gene NGS coverage over samples
...
plt_onco_mut_zoom <- plotly::ggplotly(gg_onco_mut_zoom, tooltip ="text")

```


### R2.2.2 OncoKB impact by gene (mutations)

The following plot shows each gene by the percentage of tests showing an alteration before and after oncoKB filtering.  The colors represent the number of samples tested for the gene.

**Hover over** each plot for the gene name and the same information in text.

```{r, include = T}
plt_onco_mut 
```

This is a version zoomed in on the lower left of the same plot:
