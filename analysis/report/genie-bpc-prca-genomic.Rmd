...
gg_tmb_dist_panel_onco <- plot_boxplot_by_panel(
  ...
  x_lab = "TMB (oncogenic mutations per megabase)"
)
...
### *R2.3.5 TMB by panel (oncogenic only)

We've identified a potential problem with TMB: tumor-normal samples may have a systematically lower TMB than tumor-only samples.  However, some of our planned analyses might only include oncogenic mutations, which should theoretically remove many of the germline variants making it through the tumor-only pipelines.  I was curious whether TMB shows the same bias if we use only the oncogenic mutations in calculating TMB.

This will obviously remove our ability to use rule-of-thumb values for TMB.  For example, 10 is probably too high a threshold for hypermutated samples now.  However, if we wanted to exclude the top 5% in TMB, this may be somewhat useful.
...
*Note:* I want to thank **Walid Chatila** for tremendously helpful conversations on this section, including helping me understand VAF and plot design.
...
- A VAF indicates a *possible* germline variant, owing to having one copy from each parent and one of those being altered.
