...
**Observation:** Overall the pathways I was very aware of (TP53, MYC, PI3K) were frequently annotated as oncogenic.  I was surprised that the alterations with no noted pathway were more frequently annotated as oncogenic than some in this list, like RTK_RAS and HIPPO.
...
### *R2.2.2 OncoKB levels of evidence
...
mat_pathway_pct_agree <- dft_pathway_bin %>%
  select(-sample_id, -None) %>%
  pct_agree_mat(.)

gg_pathway_mat_pct_agree <- plot_mat_metric(
  mat_pathway_pct_agree,
  fill_label = "Percentage Agreement",
  plot_title = "Pathway co-occurence"
)

mat_pathway_mut_excl <- dft_pathway_bin %>%
  select(-sample_id, -None) %>%
  test_mut_excl(.)

gg_pathway_mat_mut_excl <- plot_mat_metric(
  mat_pathway_mut_excl,
  fill_label = "Mutual exclusivity test [-log10(p-value)]",
  plot_title = "Pathway co-occurence",
  vir_opt = "viridis",
  log_fill = T
)
...
### *R2.4.1 Co-occurence, pathway

This section is a work in progress.

The first plot shows a simple, easy to interpret metric:  Percentage agreement between two variables indexing over the samples.  This is for sanity checking the test results, it should not go in a publication.  The percentage agreement is high because most samples do not have an alteration for any given pathway (so they agree):

```{r, include = T}
gg_pathway_mat_pct_agree
```

This second plot shows a test of mutual exclusivity (and the associated P value).  This uses the Poisson-binomial distribution, which differs from the default `maftools` implementation.  The motivation is taking into account that some samples have a higher mutation rate than others.

```{r, include = T}
gg_pathway_mat_mut_excl
```

The work in progress part is getting this into a divergent color scale which makes mutual exclusivity and co-occurence more readable to our audience.
...
mat_top_gene_pct_agree <- dft_top_gene_bin %>%
  select(-sample_id) %>%
  pct_agree_mat(.)

gg_top_gene_mat_pct_agree <- plot_mat_metric(
  mat_top_gene_pct_agree,
  fill_label = "Percentage Agreement",
  plot_title = "Gene alteration co-occurence",
  rotate_x_labs = T
)

mat_top_gene_mut_excl <- dft_top_gene_bin %>%
  select(-sample_id) %>%
  test_mut_excl(.)

gg_top_gene_mat_mut_excl <- plot_mat_metric(
  mat_top_gene_mut_excl,
  fill_label = "Mutual exclusivity test [-log10(p-value)]",
  plot_title = "Gene alteration co-occurence",
  vir_opt = "viridis",
  log_fill = T,
...
### *R2.4.2 Co-occurence, top genes

Again, we start with the simple metric for sanity check:

```{r, include = T}
gg_top_gene_mat_pct_agree
```

And the unadjusted P value testing mutual exclusivity:

```{r, include = T}
gg_top_gene_mat_mut_excl
```
