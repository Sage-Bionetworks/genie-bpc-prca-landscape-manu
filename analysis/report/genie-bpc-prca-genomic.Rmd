...
- A later section (currently R2.4.6) shows this chart again for 
...
The following table states the median TMB by institution/sample_type (visible in chart):
...
*Note:* I want to thank **Walid Chatila** for tremendously helpful conversations on this section, including helping me understand VAF and his design of the density plots below.
...
There are some helpful heuristics for VAF:
...
- Very low VAFs (5% for example) may indicate subclonal mutations, VAFs between 5 and 50% would be typically for alterations only found in the tumor.
...
- Models (not reported) gave similar conclusions:  MSK is either not notably different.
- A new rule of thumb for hypermutated could possibly be estimated by looking at MSK's participants (comparing their raw TMB to a TMB calculated with only oncogenic mutations).
- This will obviously remove our ability to use rule-of-thumb values for TMB.  For example, 10 is probably too high a threshold for hypermutated samples now.  More on this later.
...
- *Find a classifier cut point for TOMB*.  Using tumor-normal samples, find a metric-optimal cutpoint for TOMB which classifies samples into hypermutated (yes/no) for raw TMB.  Then apply this to all samples on TOMB (where things look more normalized).
  - Result:  **TOMB >= 5.9** is a good cut point using most metrics (F-measure, accuracy, and Cohen's kappa all agree).
...
**Follow-up** Question for the group:  Does using a hypermutated threshold based on an oncogenic calculation of TMB seem tenable for this paper?  Keeping in mind that the applications of a hypermutated label are currently very limited.
