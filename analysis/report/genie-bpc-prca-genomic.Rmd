...
gg_tmb_dist_panel <- plot_boxplot_by_panel(
  dat = dft_tmb,
  x_var = 'tmb_Mb',
...
A number of clinical characteristics were shown to differ by site:  for example, compared with MSK, DFCI had more white participants, more early stage patients (who would yield fewer metastatic samples).  Below is the fit of a linear model for log10(TMB), using patient age, panel size (log10 of the combined exon size in the panel), indicators for race, indicators for sample type, indicators for institution.
...
- This model isn't great.  TMB is zero inflated and if we wanted to push this further or publish it we should think more about our model. My only goal here was to quickly investigate whether cohort characteristics or panel size totally explained away the disparity before digging deeper.
...
### *R2.4.5 OncoKB effect on VAF

*Note:* I want to thank **Walid Chatila** for tremendously helpful conversations on this section, including helping me understand VAF.  Credit also to him for the design of the density plots below (for the Breast cancer cohort), which have been previously shared at GENIE meetings.

We will define variant allele frequency (VAF) as the proportion of reads at a genomic locus which support the presence of a variant.  Reference sequence reads are the reads which would **not** support a variant, so this is just the count of alteration reads over the total number of reads (using `t_alt_count` and `t_ref_count` from the MAF file).
...
- The tumor-only sites have a large peak at 50% VAF, indicating we likely have many mutations which are germline variants in the data.
...
- I previously stated the breast cancer cohort may have thresholded by VAF for some analyses.  This is incorrect.  In the end we applied oncogenic filtering, which (as above) limits the presence of germline variants as a side effect.
...
```{r}
gg_tmb_dist_panel_onco <- plot_boxplot_by_panel(
  dat = dft_tmb,
  x_var = 'tmb_Mb_onco',
  plot_title = "Tumor mutation burden sample distribution",
  plot_subtitle = "only including oncogenic mutations",
  x_lab = "TOMB (oncogenic mutations per megabase)"
)
...
### *R2.4.6 OncoKB effect on TMB

We've identified a sign from TMB which indicates a possible problem in our data: tumor-normal samples may have a systematically lower (and more typical) TMB than tumor-only samples.  We now investigate whether this persists when we limit our pool of alterations to only oncogenic ones.  I will call TMB calculated using only oncogenic alterations "tumor oncogenic mutation burden" (TOMB), just to establish a short hand for this report (not committed to using that term beyond).
...
- In both plots, MSK looks comparable to the other institutions when we only use oncogenic mutations in calculating TMB.
- Models similar to R2.3.4 (not reported) gave similar conclusions:  MSK is not notably different from other institutions.
...
```{r}
dft_tmb_tomb <- dft_cpt %>%
  select(record_id, cpt_seq_assay_id, contains('tmb')) %>%
  left_join(
    ., 
    distinct(select(dft_gp_all, cpt_seq_assay_id, calling_strategy)),
    by = "cpt_seq_assay_id"
  )

gg_cut_points <- ggplot(
  dft_tmb_tomb,
  aes(x = tmb_Mb_onco + 0.1, y = tmb_Mb + 0.1, color = calling_strategy)
) + 
  ...
  theme_bw() +
  labs(
    x = "TOMB (oncogenic)", y = "TMB (raw)",
    title = "TMB, TOMB and proposed cut points",
    subtitle = "Right of the black lines would be 'hypermutated'"
  ) +
  scale_color_vibrant() + 
  theme(
    plot.title.position = 'plot'
...
### *R2.4.7 New hypermutated threshold?

A priori we discussed using a TMB threshold of 10 mut/Mb to identify hypermutated individuals.  In the above work we established that we can possibly alleviate some of the bias between tumor-only and tumor-normal samples if we only consider oncogenic alterations.  If we wanted to use this, we would need to establish a new threshold which uses TOMB in place of TMB.  Two simple approaches which seem reasonable to me:

- *Percentile exclusion*. Using tumor-normal samples, find the proportion of samples (X) which are over a TMB of 10.  Declare the top X% of samples in the whole cohort as hypermutated by TOMB.
  - Result:  Excluding just over 6% of samples, which is the proportion of tumor-normal samples with a raw TMB over 10, would eliminate those with a **TOMB >= 4.7**.
- *Find a classifier cut point for TOMB*.  Using tumor-normal samples, find a metric-optimal cutpoint for TOMB which classifies samples best into hypermutated/not for TMB.  Then apply this to all samples on TOMB.
  - Result:  **TOMB >= 5.9** is a good cut point using most metrics (F-measure, accuracy, and Cohen's kappa all agree).  Even weight on sensitivity and specificity suggested TOMB >= 4.7.  Multiple metrics were assessed due to my low expertise on this topic.

Here's a plot that shows TMB vs TOMB and those two stated cut points:

```{r, fig.height = 4, include = T}
gg_cut_points
```

**Follow-up:** What does our group think about using a hypermutated threshold based on an oncogenic calculation of TMB?  Consequences are currently minor since only one analysis uses a binary indication of hypermutation, and the cost of explaining this concept may be fairly high.
...
- While "mutually exclusive" is commonly use in these plots it's incorrect for events can possibly occur together - hence the rename suggestion.
...
As a sensitivity analysis, we discussed repeating the previous plot on only subjects who had panels where those top 10 genes were tested.  This reduces the number of samples from `r n_cell_gene_assoc` in each cell of the previous plot to `r n_cell_gene_assoc_sens` here.

The axis order here mimics the "main" analysis, even though the ordering of the raw counts is different (e.g. TMPRSS2 now has more alterations than TP53, but we keep them in the order they previously appeared).  The goal is facilitating easier visual comparison.
...
- Some of the weakly significant results from the previous plot have faded here.  The strong results generally remain.
...
dft_gene_assoc_compare <- dft_gene_assoc_sens %>%
  select(
    var1, 
    var2, 
...
### *R2.6.1c Sensitivity: Direct visual comparison

I found the previous two plots hard to use, flipping back and forth over big blocks of text.  This is just a visual alternative.  Here we show the change in association for any result which was statistically significant in either R2.6.1a ("Main") or R2.6.1b ("Sensitivity"). 
...
- Generally the blue/orange dots on each line being close to each other indicates our analyses weren't dramatically altered between the main and sensitivity analyses.
- All of the negative associations (e.g. TMPRSS2:SPOP) are stronger in the sensitivity analysis than the main one, and there's overall a general trend toward lower odds ratios.
  - I think this makes sense for a collection of events with probability << 0.5, where there's more "off diagonal" junk in the main analysis (referring to the usual 2x2 contingency table).
...
dft_cpt_no_hyper_mut <- dft_cpt %>% 
  filter(tmb_Mb_onco < 5.9) # see above on threshold.
...
**Update:** Updated the definition of hypermuted to only include those with TOMB < 5.9 (TOMB = TMB only considering oncogenic mutations).  The effect is similar.
...
- We hypothesized that the complete dominance of positive pathway associations in the previous plot might be due to hypermutated samples.  
- As expected, with the hypermutated samples removed we see fewer significant results and some negative association estimates (none significant).
...
```{r}
dft_gene_assoc_spop_brca2 <- test_fisher_co_occur(
  dat = select(dft_top_gene_bin, sample_id, BRCA2, SPOP),
  ignore_cols = c("sample_id"),
  top = 2,
  alpha = 0.05
)

dft_gene_assoc_spop_brca2 %>% glimpse
```

### *SPOP/BRCA2

Irbaz asked about a specific co-occurrence which had been seen in the literature:  SPOP and BRCA2.  To do this properly we would want to expand our association framework to include genes at this level of occurence (BRCA2 is just outside the "top 10" arbitrary cutoff).

However, in the interest of exploration, we can just do the Fisher's exact test without multiple testing correction to see if there's anything there.  Apologies for the junky text output, but `ct_` variables indicate the contingency table values:

```{r, include = T}
dft_gene_assoc_spop_brca2 %>% 
  select(
    var1_lab, 
    var2_lab, 
    contains("ct_"), 
...
**Conclusions:** These are not significantly associated in our sample, and are unlikely to be under any (other) reasonable method.  There are only 4 samples which had both an SPOP and BRCA2 alteration.
...
dft_alt %>% 
  count(hugo, alt_type) %>% 
  pivot_wider(names_from = 'alt_type', values_from = 'n') %>%
  mutate(across(
    .cols = -hugo,
    .fns = \(x) if_else(is.na(x), 0, x)
  )) %>%
  mutate(n_hugo = Mutation + Fusion + CNA) %>%
  arrange(desc(n_hugo), hugo)
```
