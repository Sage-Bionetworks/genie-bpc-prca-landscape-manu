...
- Aside from report date, there are other times measured.  These include `cpt_order_int`, which is unfortunately not complete (especially for UHN), and `dx_path_proc_cpt_yrs`, which is complete but not closely related to cohort entry time.
