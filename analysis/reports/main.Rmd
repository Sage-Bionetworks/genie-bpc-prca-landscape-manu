...
dft_demo %<>% 
  mutate(
    `Year of birth` = as.integer(birth_year),
    `Year of diagnosis` = floor(birth_year + age_dx)
  ) %>%
  select(-birth_year, -age_dx) %>%
...
### R1.4e Abi/Enza and Docetaxel (all regimens)

Irbaz Riaz expressed interest the order of drugs used for participants who used either Abiraterone/Enzalutamide or Docetaxel. The cohort we examine here is anyone who used any of these drugs.  The two categories are:

1. **(Abi/Enza)** Abiraterone Acetate or Enzalutamide 
...
- Abi/Enza is a more popular first line therapy.
- The most frequent second line therapy after Abi/Enza is doing another round of Abi/Enza.


### R1.4f Abi/Enza and Docetaxel (repeats removed)

This plot is the same as the previous, except that regimens with repeat enza/abi/doce information are removed.  

For example, if a participant's regimen order was "Abi/Enza, Abi/Enza, Docetaxel" we simplify this to "Abi/Enza, Docetaxel".  But if it was "Abi/Enza, Docetaxel, Abi/Enza" then we keep the full detail (no repeats exist).
...
### R1.5b: Swimmer plot, CRPC/HSPC, dx, stage grouping
...
### R1.5c: Swimmer plot, CRPC/HSPC, by Age
...
### R1.5d: Swimmer plot, Distant metastasis status, from dx
...
### R1.5e: Swimmer plot, CRPC/HRPC by site
...
There are quite a few testosterone/PSA rows with no units.  This was shared with Jennifer Hoppe and Mike Fiandalo at AACR in April 2023:
